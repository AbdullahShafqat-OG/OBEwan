...
app.get("/api/get-plo/:id", async (request, response) => {
  const plo = await ploModel.find({ name: request.params.id });
...
app.patch("/api/update-plo/:id", async (request, response) => {
  try {
    const plo = await ploModel.findOneAndUpdate(
...
app.delete("/api/delete-plo/:id", async (request, response) => {
  try {
    const plo = await ploModel.findOneAndDelete({ name: request.params.id });
